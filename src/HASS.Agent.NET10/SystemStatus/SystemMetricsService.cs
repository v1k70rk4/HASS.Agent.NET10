...
internal sealed class SystemMetricsService : IDisposable
{
        ...
        IReadOnlySet<SensorPollingProfile>? dueProfiles = null)
    {
        ...
        var message = new SystemMetricsMessage(
            ...
            ActiveWindow: updateFast && _includeInteractiveMetrics ? LimitState(activeWindow.Title) : previous?.ActiveWindow ?? string.Empty,
            ActiveProcess: updateFast && _includeInteractiveMetrics ? LimitState(activeWindow.ProcessName) : previous?.ActiveProcess ?? string.Empty,
            ForegroundAppTitle: updateFast && _includeInteractiveMetrics ? BuildForegroundAppTitle(activeWindow) : previous?.ForegroundAppTitle ?? string.Empty,
            ...
            AudioOutputDevice: updateNormal && _includeInteractiveMetrics ? Safe(() => LimitState(_audioEndpointService?.GetOutputDeviceName() ?? string.Empty), previous?.AudioOutputDevice ?? string.Empty) : previous?.AudioOutputDevice ?? string.Empty,
            ...
            ActiveDisplay: updateNormal && _includeInteractiveMetrics ? Safe(() => FormatDisplayState(_lastDisplays), previous?.ActiveDisplay ?? string.Empty) : previous?.ActiveDisplay ?? string.Empty,
            ...
            UserPresent: updateFast && _includeInteractiveMetrics ? session.State == "active" && sessionLocked is false && !string.IsNullOrWhiteSpace(session.User) : previous?.UserPresent ?? false,
